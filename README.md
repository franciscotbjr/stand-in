...
A native desktop MCP client built entirely on **GPUI** (retained-mode GPU UI) that consumes the `stand-in-client` SDK. Inspect and interact with any MCP server, 100% local — no webview, no telemetry.

### Design System — `stand-in-mcp-explorer-ds`

A themed, token-driven component library with 24 components mirroring the canonical Design System in `stand-in-client-prototipo/`:

- **core/** — Icon (22 glyphs), Button, IconButton, Badge, CopyButton, ToggleLink, StatusDot, Spinner
...
- **theme/** — jandi colour palette mapped to `gpui-component`'s `ThemeColor`, density (compact/regular/comfy), embedded typefaces (Hanken Grotesk + JetBrains Mono)

### Gallery Storybook

Explore every component interactively in dark/light mode with density switching:

```bash
cargo run -p stand-in-mcp-explorer-gallery
```

> The Explorer app is being rebuilt on this Design System (026). The previous egui Studio has been retired — the new path uses GPUI / `gpui-component` (retained-mode). The DS crate and gallery are `publish = false` — they ship as part of the workspace, not as crates.io packages.
