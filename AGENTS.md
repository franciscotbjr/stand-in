...
To **refresh** methodology, prompts, or agent rules from upstream, use the update-project prompt from https://github.com/franciscotbjr/stateful-spec.
...
The following operations are available as Claude Code commands (invoke with `/name`):
...
- Create `.stateful-spec/history/NNN-[kebab-name].md` using the iteration template, where `NNN` is the next number after existing files in `history/`.
...
If the session **starts with a direct task** (e.g. "implement this plan") instead of a full `/resume-session` dialog, still create or attach to an iteration file **before** implementing — see the direct-task entry section in `/resume-session`.
